...
import { type Address, createWalletClient, custom, http } from "viem";
...
export function LensProvider({ children }: { children: ReactNode }) {
  ...
  const [client, setClient] = useState<PublicClient>(
    PublicClient.create({ environment: protocolTestnet })
  );
  ...
  const fetchUserAccounts = async () => {
    console.log("fetchUserAccounts", address);
    if (!client) {
        ...
        .login({
          ...
          signMessage: async (message) =>
            walletClient.signMessage({
              message,
              account: evmAddress(address as `0x${string}`),
            }),
        })
        .match(
          (result) => result,
  ...
  const createAccount = async ({
    ...
    const pictureUri = await storageClient.uploadFile(
    ...
    const { uri } = await storageClient.uploadFile(
      new File([JSON.stringify(metadata)], `metadata-${id}.json`, {
    ...
    const created = await createAccountWithUsername(sessionClient, {
      metadataUri: uri,
      accountManager: [evmAddress(address as `0x${string}`)],
      username: {
        localName: `${username}`,
      },
    })
  ...
  const createApp = async () => {
    ...
    const metadata = app({
      ...
      description:
        "Create AI-driven agents with unique attributes, all on the Lens Network. Battle, explore, and trade in a decentralized world. Each agent is unique, powered by on-chain wallets, and shaped by user-provided attributes and personalities.",
...
const LensContext = createContext<LensContextType>({
  ...
  createAccount: () =>
    Promise.resolve({ username: { value: "" }, address: "" }),
